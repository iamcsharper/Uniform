import { ChangeEvent, FC, forwardRef, useCallback, useMemo, useRef, useState } from 'react';

import { FormControlSize, FormControlTheme, FormControlVariant } from '@root/FormControl';
import {
    BaseSelect,
    BaseSelectProps,
...
import useSelectClear from '@root/Select/presets/useSelectClear';

import DefaultTagList from './TagList';
...
export const SimpleSelectWithTags = forwardRef<HTMLDivElement, SelectWithTagsProps>(
    (
        ...
        const clearableProps = useSelectClear({ Field: TagList, CrossIcon });
...
export const createSelectWithTagsWithTheme = ({
    defaultTheme,
    defaultVariant,
    defaultSize,
    defaultCrossIcon,
    ...
    defaultCrossIcon: FC<any>;
    formControl: { defaultTheme: FormControlTheme; defaultVariant: FormControlVariant; defaultSize: FormControlSize };
}) => {
    type Return = ReturnType<typeof SelectWithTags>;

    const DefaultThemedTagList = (props: any) => <DefaultTagList theme={formControl.defaultTheme} {...props} />;

    const ThemedComponent = (({
        FormControl = DefaultThemedTagList,
        theme = defaultTheme,
        ...
        props: Omit<SelectWithTagsProps, 'TagList' | 'theme' | 'CrossIcon'> & {
            FormControl?: SelectWithTagsProps['TagList'];
            theme?: SelectWithTagsProps['theme'];
            CrossIcon?: SelectWithTagsProps['CrossIcon'];
        }
    ) => Return;

    (ThemedComponent as any).displayName = 'SelectWithTags';
